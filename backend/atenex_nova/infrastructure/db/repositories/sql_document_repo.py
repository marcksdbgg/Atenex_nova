...
class SqlDocumentRepository:
    ...
    async def create(self, doc: Document) -> Document:
        m = DocumentModel(
            ...
            source_path=doc.source_path, mime_type=doc.mime_type,
            checksum=doc.checksum, status=doc.status.value,
    ...
    async def list_by_collection(
        self, collection_id: str, offset: int = 0,
        limit: int = 50, status: DocumentStatus | None = None,
    ) -> list[Document]:
    ...
    async def count_by_collection(self, cid: str) -> int:
        r = await self._session.execute(
            select(func.count()).select_from(DocumentModel).where(
                DocumentModel.collection_id == cid
            )
        )
        return r.scalar_one()
    ...
    def _to_entity(m: DocumentModel) -> Document:
        return Document(
            id=m.id, collection_id=m.collection_id, title=m.title,
            source_path=m.source_path, mime_type=m.mime_type,
            checksum=m.checksum, status=DocumentStatus(m.status),
