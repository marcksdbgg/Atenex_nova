...
class SqlJobRepository:
    ...
    async def create(self, job: Job) -> Job:
        m = JobModel(
            id=job.id, job_type=job.job_type.value, target_id=job.target_id,
            status=job.status.value, payload_json=json.dumps(job.payload),
            retries=job.retries, max_retries=job.max_retries,
    ...
    async def list_all(
        ...
        status: JobStatus | None = None, job_type: JobType | None = None,
    ) -> list[Job]:
    ...
    def _to_entity(m: JobModel) -> Job:
        return Job(
            id=m.id, job_type=JobType(m.job_type), target_id=m.target_id,
            ...
            error=m.error, retries=m.retries, max_retries=m.max_retries,
