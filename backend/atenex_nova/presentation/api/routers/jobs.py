...
async def list_jobs(
    ...
    return [
        JobResponse(
            id=j.id, job_type=j.job_type.value, target_id=j.target_id,
            status=j.status.value, error=j.error, retries=j.retries,
...
async def get_job(
    ...
    return JobResponse(
        ...
        status=job.status.value, error=job.error, retries=job.retries,
